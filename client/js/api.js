...
const NetZeroAPI = {
  ...
  async calculate(levers, baselines) {
    try {
      ...
      if (res.ok) {
        ...
        NetZeroAPI._notifyFallbackStatus(false);
        return json.data;
      }
    } catch (err) {
    ...
    NetZeroAPI._notifyFallbackStatus(true);
  ...
  // Notify the UI when we switch between the live backend and the local
  // fallback engine, so a demo presenter can see at a glance which one is active.
  _notifyFallbackStatus(usingFallback) {
    window.dispatchEvent(new CustomEvent('netzero:backend-status', {
      detail: { usingFallback }
    }));
  },

  // ============================================================================
  // ⚠️  DUPLICATE ENGINE — READ BEFORE EDITING SERVER-SIDE CALCULATION LOGIC ⚠️
  //
  // Everything below (localCalculate, localOptimizerPackages) is a hand-kept
  // duplicate of server/engine/emissions.js, server/engine/finance.js, and
  // server/engine/optimizer.js, used ONLY as an offline fallback when the
  // Express backend is unreachable.
  //
  // If you change server/engine/emissions.js, finance.js, or optimizer.js,
  // you MUST make the identical change here, or this offline fallback will
  // silently produce numbers that disagree with the real backend.
  // Search this file for every reference to a lever name you changed
  // (e.g. adding/renaming a lever, changing a constant, changing a formula).
  // ============================================================================
  ...
  localCalculate(levers, baselines) {
    ...
    const lev = {
      ...
      fleetDieselEfficiencyPercent: 0,
      logisticsRouteOptimisationPercent: 0,
    ...
    // Scope 1
    const effectiveDiesel = Math.max(0, cfg.dieselUsageLiters * (1 - (lev.fleetDieselEfficiencyPercent / 100)));
    ...
    const scope3TransportKg = shipmentTonnes * (cfg.inboundTransportKm + cfg.outboundTransportKm) * 0.12 * (1 - (lev.logisticsRouteOptimisationPercent / 100));
    ...
    if (lev.logisticsRouteOptimisationPercent > 0) totalCapExINR += 25000;
    if (lev.fleetDieselEfficiencyPercent > 0) totalCapExINR += 15000;
  ...
  // ⚠️ DUPLICATE ENGINE — mirrors server/engine/optimizer.js. If the Quick
  // Wins / Max ROI / Deep Net Zero packages change on the server, update the
  // hardcoded lever values and figures below to match, or this fallback will
  // silently show different presets than the live backend.
    ...
    return [
      {
        ...
        levers: { compressorPressureBar: 30, scrapRecyclingPercent: 75, logisticsRouteOptimisationPercent: 20, fleetDieselEfficiencyPercent: 10 },
