...
class ChartVisualizer {
  ...
  initEmissionsTrajectory() {
    ...
    const areaColors = [
      ['rgba(245, 158, 11, 0.82)', '#f59e0b'],
      ['rgba(6, 182, 212, 0.78)', '#06b6d4'],
      ['rgba(16, 185, 129, 0.82)', '#10b981'],
      ['rgba(139, 92, 246, 0.78)', '#8b5cf6']
    ];
  ...
  update(data, baselineData) {
    ...
    if (this.emissionsChart) {
      ...
      scopeKeys.forEach((key, index) => {
        ...
        const step = (currentValue - baselineValue) / 5;
        this.emissionsChart.data.datasets[index].data = Array.from(
          { length: 6 },
          (_, yearIndex) => Math.max(0, Math.round((baselineValue + (step * yearIndex)) * 10) / 10)
        );
      });
