...
const DEFAULT_LEVERS = {
  ...
  fleetDieselEfficiencyPercent: 0,    // 0 - 25% (Scope 1: own diesel trucks/generators)
  logisticsRouteOptimisationPercent: 0, // 0 - 25% (Scope 3: inbound/outbound freight distance)
...
function calculateEmissions(levers = {}, baselines = {}) {
  ...
  // Fleet diesel efficiency slightly discounts operational diesel use
  const effectiveDieselLiters = Math.max(0, cfg.dieselUsageLiters * (1 - (lev.fleetDieselEfficiencyPercent / 100)));
  ...
  const transportEfficiencyFactor = 1 - (lev.logisticsRouteOptimisationPercent / 100);
